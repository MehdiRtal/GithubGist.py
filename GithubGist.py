...
class GitHubGist:
    ...
    def get_gist(self, id):
        r = requests.get(f"https://api.github.com/gists/{id}", headers=self.headers)
        if r.status_code != 200:
            raise Exception(r.text)
        return r.json()

    def update_gist(self, id, filename, content):
        r = requests.patch(f"https://api.github.com/gists/{id}", headers=self.headers, json={"files": {filename: {"content": content}}})
        if r.status_code != 200:
            raise Exception(r.text)
